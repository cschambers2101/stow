...
# -----------------------------------------------------------------
# 5A. NETWORK TRUST: THE OAKFORD ROOT CA
#
#    Lettered, not renumbered, so the later section numbers keep matching
#    the ones quoted elsewhere.
#
#    THE POSITION IS THE POINT. The site firewall intercepts TLS, so
#    anything fetched from outside the Ubuntu archives fails certificate
#    validation until this CA is trusted. This used to sit in section 14,
#    three sections AFTER Chrome is pulled from dl.google.com in section
#    11 -- so on the school network the build died there. Same defect as
#    the niri installer, fixed the same way, 2 Sep 2026.
#
#    It sits here rather than at line 1 because the Chrome/NSS half needs
#    `certutil`, from `libnss3-tools` in section 5. Sections 1-5 reach
#    only the Ubuntu archives, whose traffic bypasses the firewall, as
#    does oakfordhelp.co.uk itself -- which is what lets the download
#    below succeed before its own certificate is trusted.
#
#    Keep every third-party download BELOW this line: 11 (Chrome),
#    16 (yt-dlp), 17 (tpm), 18 (Claude Code), 19 (Antigravity).
# -----------------------------------------------------------------
...
# -----------------------------------------------------------------
# 14. S6C WIFI PROFILE
#     The Oakford CA moved to section 5A on 2 Sep 2026 -- it has to be
#     trusted before section 11 fetches Chrome from dl.google.com.
# -----------------------------------------------------------------
