#!/usr/bin/env bash
# setup-printing: build the CUPS queues for a Xerox AltaLink finisher + PaperCut. Site values come from
# ~/.config/s6c-printing.conf (see --sample). Usage: bash setup-printing.sh [--packages] [--ppd] [--queues] [--verify] [--all]
...
PAPERCUT_IP="${PAPERCUT_IP:-}"
PAPERCUT_USER="${PAPERCUT_USER:-}"
TRAY_FLAT="${TRAY_FLAT:-Tray1}"
TRAY_A3_SEF="${TRAY_A3_SEF:-Tray2}"
TRAY_A4_SEF="${TRAY_A4_SEF:-Tray3}"
